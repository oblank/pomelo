...
var bizlogger = require('../../../node_modules/pomelo-logger').getLogger('pomelo-biz', __filename);
...
SessionService.prototype.create = function(sid, frontendId, socket) {
  ...
  //bizlogger.info('【create session id】：' + session.id + ', serverId: ' + frontendId);
  return session;
};
...
SessionService.prototype.remove = function(sid) {
  var session = this.sessions[sid];
  //bizlogger.info('【remove session id】: ' + sid + ', serverId: ' + (session ? session.frontendId : '无'));
  if(session) {
    var uid = session.uid;
...
SessionService.prototype.sendMessageByUid4Mobile = function(uid, msg) {
        var sessions = this.uidMap[uid];
        if(!sessions) {
            logger.debug('fail to send message by uid for non-existing session. uid: %j',
                uid);
        ...
        for(var i=0, l=sessions.length; i<l; i++) {
            ...
            logger.info('****sendMessageByUid4Mobile, client_os = %j.', sessions[i].get("client_os"));
            //logger.info('****sendMessageByUid4Mobile, session', sessions[i]);
...
FrontendSession.prototype.pushAll = function(cb) {
  //bizlogger.info('【pushAll session id】: ' + this.id + ', serverId: ' + this.frontendId);
