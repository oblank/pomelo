...
//向 /root/pomelo_history.log 写入成功标识，以便bash检测pomelo是否成功启动
var fs = require('fs');
function writeSuccessSign(){
    var file = '/root/pomelo_history.log';
    fs.exists(file, function(exists){
        if(exists){
	    var d = utils.format(new Date(), 'yyyy-MM-dd hh:mm:ss') + ' @watchdog restart pomelo success\n';
            fs.appendFile(file, d, 'utf8', function(err){
                if(err){
                    return console.error('appendFile failed');
                }
            });
        }
    });
